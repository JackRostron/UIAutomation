...
var currentVersion = target.host().performTaskWithPathArgumentsTimeout("/bin/bash", ["./fileUpdated.sh"], 90);
var loopVersion;
var command;

while ( running == 0 ) {
	//UIALogger.logMessage("looped");
	loopVersion = target.host().performTaskWithPathArgumentsTimeout("/bin/bash", ["./fileUpdated.sh"], 10);
	...
	if ( parseInt(currentVersion.stdout) < parseInt(loopVersion.stdout) ){
		...
		command = target.host().performTaskWithPathArgumentsTimeout("/bin/cat", ["./IATUtilities.js"], 10);
		//eval(command.stdout);
